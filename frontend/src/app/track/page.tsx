...
export default function TrackerPage() {
  ...
  const analyzeFoodStr = async () => {
    ...
    setParsedDraft(null);
    try {
      const res = await fetch(`${API}/api/ai-parse-food`, {
        method: 'POST',
  ...
  const handleGetRecommendation = async () => {
    setIsAiLoading(true);
    try {
      const response = await fetch(`${API}/api/ai-recommend`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
  ...
  const consumedCalories = consumedMeals.reduce((acc, log) => acc + log.meal.calories, 0) + 650;
  ...
  useEffect(() => {
    if (user?.id) {
      fetch(`${API}/api/checkout/${user.id}/orders`)
        .then(res => res.json())
        .then(data => {
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
              ...
              {!parsedDraft ? (
                <div className="space-y-4">
                  ...
                  <button 
                    ...
                    className="w-full py-3 bg-neutral-900 text-white rounded-xl text-sm font-bold shadow-lg hover:bg-neutral-800 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Analyze Macros
                  </button>
                </div>
                ...
                <div className="space-y-4 bg-green-50 p-4 rounded-2xl border border-green-100">
                  ...
                  <div className="flex gap-2 mt-4">
                    <button onClick={() => setParsedDraft(null)} className="w-full py-2 bg-white dark:bg-neutral-900 text-green-900 font-bold rounded-xl border border-green-200 hover:bg-neutral-50 transition-colors text-sm">Discard</button>
            ...
            <div className="bg-neutral-900 p-8 rounded-3xl text-white shadow-xl relative overflow-hidden flex flex-col justify-between">
              <div className="relative z-10 w-full">
                ...
                {!aiRecommendations || aiRecommendations.length === 0 ? (
                  ...
                  <div className="bg-white/5 backdrop-blur border border-white/10 rounded-2xl p-6 relative">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      <div className="flex gap-2 items-center bg-black/40 rounded-full px-2 py-1">
                        <button onClick={() => setCurrentRecIndex(prev => Math.max(0, prev - 1))} disabled={currentRecIndex === 0} className="text-white hover:text-green-400 disabled:opacity-30">
                        ...
                        <button onClick={() => setCurrentRecIndex(prev => Math.min(aiRecommendations.length - 1, prev + 1))} disabled={currentRecIndex === aiRecommendations.length - 1} className="text-white hover:text-green-400 disabled:opacity-30">
                    ...
                    <div className="flex justify-between items-center pt-4 border-t border-white/20">
                      <span className="font-bold text-neutral-300">Basket Total: <span className="text-white text-xl ml-1">₹{aiRecommendations[currentRecIndex].totalPrice}</span></span>
                      <div className="flex gap-2">
                        <button onClick={() => setAiRecommendations([])} className="px-4 py-2 font-bold text-xs bg-white/10 hover:bg-white/20 rounded-xl transition-colors">Discard</button>
                        <button onClick={logAiMeal} className="px-5 py-2 font-bold text-xs bg-green-500 text-white rounded-xl hover:bg-green-400 transition-colors">Log & Deduct</button>
                      </div>
                    </div>
                  </div>
                )}
      ...
      {activeTab === 'orders' && (
        <div className="space-y-6">
          {orders.length === 0 ? (
            <div className="text-center py-16 text-neutral-400 font-bold">No orders yet.</div>
          ) : orders.map((order) => (
            <div key={order.id} className="bg-white dark:bg-neutral-900 rounded-3xl border border-neutral-100 dark:border-neutral-800 shadow-sm overflow-hidden">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div className="text-sm text-neutral-600 dark:text-neutral-400 max-w-sm">{order.items}</div>
