...
def seed():
    ...
    # Menu Items
    db.session.add_all([
        MenuItem(id="f1", name="Avocado Toast with Poached Egg",  description="Whole grain toast topped with fresh avocado and a soft poached egg.",             category="Breakfast", priceInPaise=35000, calories=350, protein=14, carbs=28, fat=20, type="Non-veg", image="https://images.unsplash.com/photo-1525351484163-7529414344d8?w=800&q=80", isPopular=True,  kitchenId="r4"),
        MenuItem(id="f2", name="Vegan Protein Oat Bowl",          description="Oats loaded with chia seeds, berries, and almond butter.",                        category="Breakfast", priceInPaise=25000, calories=420, protein=20, carbs=55, fat=16, type="Vegan",   image="https://images.unsplash.com/photo-1517673132405-a56a62b18caf?w=800&q=80", isPopular=False, kitchenId="r4"),
        MenuItem(id="f3", name="Grilled Chicken Quinoa Salad",    description="Lean chicken breast strips over a bed of quinoa, cherry tomatoes, and cucumber.", category="Lunch",     priceInPaise=45000, calories=480, protein=45, carbs=40, fat=15, type="Non-veg", image="https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800&q=80", isPopular=True,  kitchenId="r1"),
        MenuItem(id="f4", name="Lentil & Sweet Potato Curry",     description="Rich, warming curry packed with plant-based protein and fiber.",                   category="Lunch",     priceInPaise=39000, calories=410, protein=18, carbs=65, fat=10, type="Vegan",   image="https://images.unsplash.com/photo-1565557612666-ac563604e389?w=800&q=80", isPopular=False, kitchenId="r3"),
        MenuItem(id="f5", name="Baked Salmon with Asparagus",     description="Omega-3 rich salmon fillet baked with lemon and served with roasted asparagus.",   category="Dinner",    priceInPaise=89000, calories=450, protein=42, carbs=10, fat=26, type="Non-veg", image="https://images.unsplash.com/photo-1467003909585-2f8a72700288?w=800&q=80", isPopular=True,  kitchenId="r2"),
        MenuItem(id="f6", name="Paneer Tikka Wrap",               description="Whole wheat wrap featuring spiced grilled paneer, peppers, and mint chutney.",     category="Snacks",    priceInPaise=22000, calories=380, protein=16, carbs=35, fat=20, type="Veg",     image="https://images.unsplash.com/photo-1629814402631-01f1437198a2?w=800&q=80", isPopular=False, kitchenId="r3"),
        MenuItem(id="f7", name="Protein Bliss Bites",             description="Energy balls made from dates, nuts, and whey isolates.",                          category="Snacks",    priceInPaise=15000, calories=220, protein=12, carbs=25, fat=9,  type="Veg",     image="https://images.unsplash.com/photo-1528751014936-863e6e8a3ee2?w=800&q=80", isPopular=False, kitchenId="r2"),
    ])

    # Daily stats + logs (7 days)
    today = date.today()
    now = datetime.utcnow()
    ...
    for i in range(7):
        ...
        db.session.add(LogEntry(userId="user_1", itemName="Morning Oats",  calories=350, protein=15, carbs=45, fat=10, priceInPaise=15000, loggedAt=log_time.replace(hour=8)))
        db.session.add(LogEntry(userId="user_1", itemName="Chicken Salad", calories=450, protein=40, carbs=20, fat=15, priceInPaise=35000, loggedAt=log_time.replace(hour=13)))
        db.session.add(LogEntry(userId="user_1", itemName="Dinner Soup",   calories=300, protein=10, carbs=40, fat=5,  priceInPaise=20000, loggedAt=log_time.replace(hour=19)))

    db.session.commit()
    print("Database seeded completely!")


# Allow running directly locally: python seed.py
